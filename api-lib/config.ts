...
const addRarity = (_id: any, _from: any, _to: any) => {
  const _rarityWeight = {
    value: _id,
    from: _from,
    to: _to,
...
const addRarityPercentForLayer = (_rarityId: string, _layerId: string, _percentages: { [x: string]: any; super_rare?: number; rare?: number; original?: number; }) => {
  let _rarityFound = false;
  rarityWeights.forEach((_rarityWeight:any) => {
    if (_rarityWeight.value === _rarityId) {
      const _percentArray: any = [];
